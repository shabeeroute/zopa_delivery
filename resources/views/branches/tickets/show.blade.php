                ...
                <div class="card-body">
                    <div class="d-flex align-items-center mb-4">
                        <div class="flex-shrink-0 me-3">
                            @if(!empty($ticket->branch->image))
                                <img src="{{ URL::asset('storage/branchs/' . $ticket->branch->image) }}" alt="" class="rounded-circle avatar-sm">
                            @else
                        ...
                        <div class="flex-grow-1">
                            <h5 class="font-size-14 mb-0">{{ $ticket->branch->name }}</h5>
                            <small class="text-muted">{{ $ticket->branch->email }}</small>
                        </div>
                    </div>
                            ...
                            <div class="d-flex align-items-center mb-4">
                                <div class="flex-shrink-0 me-3">
                                    @if ($reply->user_id)
                                        ...
                                        @endif --}}
                                        {{-- //TODO: check auth user here. whether admin or branch or customer --}}
                                    @else
                                        @if(!empty($ticket->branch->image))
                                            <img src="{{ URL::asset('storage/branchs/' . $ticket->branch->image) }}" alt="" class="rounded-circle avatar-sm">
                                        @else
                                ...
                                <div class="flex-grow-1">
                                    <h5 class="font-size-14 mb-0">{{ $reply->user_id ? ucfirst(auth()->guard('branch')->user()->name) :  $ticket->branch->name}}</h5>
                                    <small class="text-muted">{{ auth()->guard('branch')->user()->email }}</small>
                                </div>
                            </div>
                            {!! $reply->description !!}
...
<div class="modal fade" id="composemodal" tabindex="-1" role="dialog" aria-labelledby="composemodalTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <form method="POST" action="{{ route('branch.tickets.reply.store')  }}">
                @csrf
